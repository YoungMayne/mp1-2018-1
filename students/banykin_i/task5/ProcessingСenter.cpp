#include "ProcessingСenter.h"

ProcessingСenter::ProcessingСenter() {
	LoadFromFile();
}

void ProcessingСenter::Working(int number) {
...
int ProcessingСenter::Month(std::string month) {
	...
	if (month == "Авгус") return 8;
	if (month == "Сентябрь") return 9;
	if (month == "Октябрь") return 10;
	if (month == "Ноябрь") return 11;
	if (month == "Декабрь") return 12;
}

void ProcessingСenter::RegisterFromFile(number4 number, std::string _name, std::string _pass, int _money, int _day, int _month, int _year, int day, int month, int year, int deposite, double percent) {
...
void ProcessingСenter::LoadFromFile() {
